        ...
        return [
            Message(
                ...
                content=(
                    ...
                    "  - Set position with set_node_property(node_path, 'position', {'x': 0, 'y': 0})\n\n"
                    ...
                    "  - setup_collision(node_path='./MyNode', collision_node_type='CollisionShape2D',\n"
                    "                    shape_type='CircleShape2D', properties={'radius': 32})\n\n"
        ...
        return [
            Message(
                ...
                content=(
                    ...
                    f"  get_script_for_node(node_path='{node_path}') → read back the attached script\n"
                    f"  get_parse_errors(script_path='{script_path}') → final parse check\n\n"
        ...
        return [
            Message(
                ...
                content=(
                    ...
                    "  debug_workflow()\n"
                    "  → Returns bridge state, scene tree, headless run errors, and suggestions.\n\n"
                    "PHASE 2 — Script Analysis (if a script is involved)\n"
                    f"  get_parse_errors(script_path='{script_path or 'res://scripts/my_script.gd'}')\n"
                    "  → Fix any parse errors (syntax, missing parentheses, undefined variables).\n\n"
                    "PHASE 3 — Scene Structure Check\n"
                    "  get_scene_tree(max_depth=-1)\n"
                    "  get_node_properties(node_path='./Player')  [or the failing node]\n"
                    "  → Verify nodes exist, scripts are attached, and property values are sane.\n\n"
                    "PHASE 4 — Signal Flow Inspection\n"
                    f"  analyze_signal_flow(scene='{scene_path or 'res://scenes/main.tscn'}')\n"
                    "  → Look for disconnected signals, wrong method names, or missing target nodes.\n\n"
                    ...
                    "- 'Null instance' → a node was freed (queue_free) but something still references it.\n"
        ...
        return [
            Message(
                ...
                content=(
                    ...
                    "  - Look for null references, missing nodes, or signal connection failures.\n\n"
                    ...
                    "  - Check the live scene tree includes the target node (get_game_scene_tree()).\n\n"
                    "DOCUMENTATION:\n"
