...
https://hybridindie.github.io/godot-mcp/ — rebuilds on every `docs/**` push to `main` via
`.github/workflows/pages.yml` (mkdocs-material, `--strict`). For LLM agents, the site ships
...
markdown document, generated by the `mkdocs-llmstxt` plugin) — point agents at those URLs
