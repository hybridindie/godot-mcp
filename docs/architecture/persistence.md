...
```mermaid
flowchart TD
    N["target node"] --> W{"walk the owner<br/>chain to the root"}
    W -->|"owner == null"| X["not persisted<br/>node_not_owned"]
    W -->|"owner == root"| OK["persisted"]
    W -->|"owner != root"| E{"is_editable_instance<br/>(owner)?"}
    E -->|yes| OK
    E -->|no| Y["not persisted<br/>instanced_child_not_editable"]
```
