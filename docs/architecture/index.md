...
flowchart LR
    subgraph client["Agent side"]
        AI["AI client<br/>(OpenCode / Claude / any MCP client)"]
    end
    subgraph server["godot-mcp server (Python)"]
        SRV["FastMCP server<br/>184 tools · safety · Pydantic models"]
        BR["WebSocket listener<br/>ws://127.0.0.1:9080"]
    end
    subgraph addon["godot-mcp addon (GDScript)"]
        WS["WebSocketPeer client<br/>connects out · reconnects"]
        RT["Command router<br/>80+ cmd_* handlers"]
    end
