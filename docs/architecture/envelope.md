...
```mermaid
flowchart LR
    A["tool call<br/>(MCP)"] --> B["safety: preconditions,<br/>dry_run/confirm"]
    B --> C{"dry_run?"}
    C -->|yes| D["preview + probe<br/>(cmd_node_persistence)"]
    C -->|no| E["Bridge.send<br/>{id, command, params}"]
    E --> F["addon handler<br/>(UndoRedo, Editor API)"]
    F --> G["{id, ok, result}"]
    G --> H["typed Pydantic model<br/>→ structuredContent"]
    B --> H2["ToolError<br/>{error, hint, required}"]
    G --> H3["ok:false envelope<br/>→ ToolError"]
```
