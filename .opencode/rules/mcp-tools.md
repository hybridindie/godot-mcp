...
- **New categories register gated off** — added to `TOOLSETS` but not `DEFAULT_ENABLED`. The agent turns them on with `enable_toolset` (which writes into the server-global enabled set via `ToolsetMiddleware`, issue #227/#364). Default exposure stays `core` + `inspection`. The enabled set is a single process-wide set shared by all clients — godot-mcp is a single-user local server, so per-session isolation was removed (it was dead code on the sessionless `2026-07-28` protocol real clients use, and `Middleware.on_initialize` violated `async-patterns.md`). `enable_toolset`/`disable_toolset` work on every transport.
